...
```bash
gpg --locate-keys rodrigo@teamx.agency
```

That fetches it over [Web Key Directory](https://wiki.gnupg.org/WKD) from `teamx.agency` — the domain
in the key's own address, and **an origin separate from the one serving this installer**. That
separation is what the check is made of: raw.githubusercontent.com and teamx.agency would both have to
be compromised, at the same time, to agree on a lie.

If WKD is blocked on your network, the key also ships inside the tag:

```bash
curl -fsSL https://raw.githubusercontent.com/governed-software/doctrine-driven-development/v0.1.2/KEYS.asc | gpg --import
```

That path is weaker and worth naming as such — it takes the key from the same place as what the key is
supposed to authenticate, so an attacker who controls one controls both. Cross-check it against
something we do not serve: **GitHub** marks every commit and the release tag *Verified* under
`7D72DEBDA1D36D34`, and [governed.software](https://governed.software) publishes the same fingerprint
from a third host.

Trusting a key on first sight is a decision, not a proof. This is what we can honestly offer, so you
can make it with the real terms in front of you.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/governed-software/doctrine-driven-development/v0.1.2/install.sh
curl -fsSLO https://raw.githubusercontent.com/governed-software/doctrine-driven-development/v0.1.2/SHA256SUMS
curl -fsSLO https://raw.githubusercontent.com/governed-software/doctrine-driven-development/v0.1.2/SHA256SUMS.asc
...
```bash
git clone --branch v0.1.2 https://github.com/governed-software/doctrine-driven-development
...
```bash
curl -fsSL https://raw.githubusercontent.com/governed-software/doctrine-driven-development/v0.1.2/install.sh | bash -s -- --pro
```
